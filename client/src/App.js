...
function App() {
  ...
  const submitHandler = () => {
    ...
    try {
      console.log("worked")
      var url =
        "https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&origin=*&redirects=1&titles="+term
        console.log(url);
      axios.get(url).then((response) =>{
        ...
        console.log(response.data["query"]["pages"][keys[0]]["extract"])
  ...
  const showAnalysis = () => {
    if (submitted) {
      return (
        <div class="row box shadow-lg pt-5 pb-1">
          <div class="col-sm-4">
            <Chart
              options={plotOptions}
              series={series}
    ...
    <div>
      ...
      <div class="footer my-auto">
        <p>
          ...
          {/* Made with <h1 class="fa fa-heart" style={{ color: "#f5e907" }}></h1>{" "} */}
          by S3D 
        </p>
      </div>
    </div>
